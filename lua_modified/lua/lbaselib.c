...
static int chunk_handler(lua_State *L, unsigned char *decoded, size_t dlen, int envidx, const char *mode) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "CHUYE_CHUNKS"); // Index 5
    if (lua_isnil(L, 5)) {
        lua_pop(L, 1);
        lua_newtable(L);
        lua_pushvalue(L, -1);
    ...
    if (lua_rawgeti(L, 5, (lua_Integer)whole_crc) == LUA_TNIL) { // Index 6
        lua_pop(L, 1);
        lua_newtable(L);
    ...
    if (lua_rawgeti(L, 6, (lua_Integer)index) == LUA_TNIL) {
        ...
        lua_setfield(L, 6, "count");

        lua_getfield(L, 6, "total");
        ...
        if (count == expected) {
            ...
            if (decompressed) {
                ...
                memset(decompressed, 0, decompressed_len);
                free(decompressed);
            } else {
...
static int luaB_load (lua_State *L) {
  ...
  if (s != NULL && l > 0) {
    size_t dlen;
    const char *data_to_decrypt = s;
    size_t len_to_decrypt = l;
    unsigned char *decompressed_b85 = NULL;

    if (l > 2 && s[0] == 0x1b && s[1] == 'Z') {
        size_t dec_len;
        decompressed_b85 = luaL_decompress((const unsigned char *)s + 2, l - 2, &dec_len);
        if (decompressed_b85) {
            data_to_decrypt = (const char *)decompressed_b85;
            len_to_decrypt = dec_len;
        }
    }

    unsigned char *decoded = luaL_decrypt_chuye(data_to_decrypt, len_to_decrypt, &dlen);
    if (decompressed_b85) {
        memset(decompressed_b85, 0, len_to_decrypt);
        free(decompressed_b85);
    }
...
static int luaB_chuye_normalize (lua_State *L) {
  size_t len;
  ...
  luaU_dump_clean(L2, cl->p, writer_file, f);
  fclose(f);
  lua_close(L2);
  if (decompressed) free(decompressed);
  free(decoded);
...
static const luaL_Reg base_funcs[] = {
  ...
  {"load", luaB_load},
{"chuye_normalize", luaB_chuye_normalize},
